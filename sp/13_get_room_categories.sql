CREATE PROCEDURE get_room_categories(
...
BEGIN 
        ...
        SET
          @query = CONCAT(
            "SELECT  r.name, r.slug, r.color, r.image, r.description, ",  cc ," cc ,  
			JSON_ARRAYAGG(JSON_OBJECT('id',rc.id, 'description',rc.description,'icon',rc.icon)) detail
             FROM api_roomcategory r 
			inner join api_roomcategorydetail rc on rc.room_category_id = r.id
            WHERE ",
